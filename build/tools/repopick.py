...
if __name__ == '__main__':
    # Default to SEREIN Gerrit
    default_gerrit = 'https://review.sereinrom.com'
    ...
    for item in mergables:
        ...
        if args.gerrit == default_gerrit:
            ...
            if args.pull:
                cmd = ['git pull --no-edit serein', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch serein', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
        ...
        if args.gerrit != default_gerrit or os.stat(FETCH_HEAD).st_size == 0:
            # If not using the default gerrit or serein failed, fetch from gerrit.
            if args.verbose:
                if args.gerrit == default_gerrit:
