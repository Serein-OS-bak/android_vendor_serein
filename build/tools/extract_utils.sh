...
function setup_vendor() {
    ...
    export SEREIN_ROOT="$3"
    if [ ! -d "$SEREIN_ROOT" ]; then
        echo "\$SEREIn_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SEREIN_ROOT/$OUTDIR" ]; then
        mkdir -p "$SEREIN_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SEREIN_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$SEREIN_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SEREIN_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SEREIN_ROOT"/vendor/serein/build/tools/smali/baksmali.jar
        export SMALIJAR="$SEREIN_ROOT"/vendor/serein/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$SEREIN_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SEREIN_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$SEREIN_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$SEREIN_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$SEREIN_ROOT"/vendor/serein/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SEREIN_ROOT"/"$OUTDIR"/radio
